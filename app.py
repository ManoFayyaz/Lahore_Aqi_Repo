...
def prediction_chart(): 
    ...
    daily_max = [df["calculated_aqi"][i*24:(i+1)*24].max() for i in range(3)]
    ...
    plt.title("Maximum AQI for Next 3 Days") 
    plt.xlabel("Date") plt.ylabel("Max AQI") 
    # plt.grid(True) 
    plt.tight_layout()
    return plt.gcf() 
...
def pm2_5_line_chart(): 
    ...
    ax.set_title("Maximum PM2.5 Forecast (Next 3 Days)") 
    plt.tight_layout() 
    return fig 
...
def alert_dataframe(): 
    ...
    alerts = [] for date, value in zip(days, daily_max):
        if value >=0 and value<=50: 
            ...
            level="Hazourdous" 
        alerts.append({"Date": date, "Max PM2.5": value, "AQI Category": level}) 
...
with gr.Blocks() as dashboard: 
        ...
        with gr.Row(): 
            with gr.Column():
                gr.Plot(value=pm2_5_line_chart(), label="PM2.5 Forecast (Next 3 Days)") 
        ...
        with gr.Row(): 
            with gr.Column(): 
                 gr.Plot(value= prediction_chart(), label="Max AQI") 
